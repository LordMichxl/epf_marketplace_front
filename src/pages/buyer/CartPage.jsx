      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 h-fit sticky top-24">
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 mb-1 block">Code coupon</label>
              <input
                type="text"
                value={coupon}
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl shadow-sm p-4 flex items-center gap-4">
                ...
                {/* Quantité */}
                <div className="flex items-center border border-gray-200 rounded-lg">
                  <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-2 py-1 text-gray-600 hover:text-indigo-600">
                    <Minus size={16} />
                  </button>
                  <span className="px-3 py-1 font-medium">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-2 py-1 text-gray-600 hover:text-indigo-600">
